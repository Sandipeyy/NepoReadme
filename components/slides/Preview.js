...
export default function Preview({ back }) {
  ...
  useEffect(() => {
    db.collection(nepoStore.data.username).add({
      date: Date(),
      data: nepoStore.data.finalData,
    });
    setTimeout(() => {
      document.getElementById("content").innerHTML = md.render(
        nepoStore.data.finalData
      );
  ...
  function onCopy() {
    navigator.clipboard.writeText(nepoStore.data.finalData);
    // Alert for Copied
    copied();
  }
  function onDownload() {
    const element = document.createElement("a");
    const file = new Blob([nepoStore.data.finalData], {
      type: "text/plain",
    });
