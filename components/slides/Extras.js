...
export default function Extras({ back }) {
  ...
  const nepStore = useNepoStore();
  const [theme, setTheme] = useState(nepStore.data.trophy.theme);
  const [border, setBorder] = useState(nepStore.data.trophy.border);
  const [background, setBackground] = useState(
    nepStore.data.trophy.background
  );
  const [quoteTheme, setQuoteTheme] = useState(nepStore.data.quote.quoteTheme);
  const [layout, setLayout] = useState(nepStore.data.quote.layout);
  const [color, setColor] = useState(nepStore.data.visitcount.color);
  const [icon, setIcon] = useState(nepStore.data.visitcount.icon);
  const [topRepoTheme, setTopRepoTheme] = useState(
    nepStore.data.toprepo.toprepotheme
  );
  useEffect(() => {
    nepStore.data.trophy.theme = theme;
  }, [theme]);
  useEffect(() => {
    nepoStore.data.trophy.border = border;
  }, [border]);
  useEffect(() => {
    nepStore.data.trophy.background = background;
  }, [background]);
  useEffect(() => {
    nepStore.data.visitcount.color = color;
  }, [color]);
  useEffect(() => {
    nepStore.data.visitcount.icon = icon;
  }, [icon]);
  useEffect(() => {
    nepStore.data.quote.quoteTheme = quoteTheme;
  }, [quoteTheme]);
  useEffect(() => {
    nepStore.data.quote.layout = layout;
  }, [layout]);
  useEffect(() => {
    nepStore.data.toprepo.toprepotheme = topRepoTheme;
  }, [topRepoTheme]);
...
`;
    ...
    //         `
    // ## 🐦 Latest Tweet
    // [![](https://gtce.itsvg.in/api?username=${nepoStore.data.socials.x})](https://github.com/VishwaGauravIn/github-x-card-embed)
    // `;
    //     }
...
//     }
    if (document.getElementById("visitorschk").checked === true) {
      extras =
        ...
        `
---
[![](https://visitcount.itsvg.in/api?id=${nepoStore.data.username}&icon=${icon}&color=${color})](https://visitcount.itsvg.in)
`;
    }
  ...
  function createFinalData() {
    var finaldata = "";
    if (nepoStore.data.aboutme != ``) {
      finaldata =
        finaldata +
        `# 💫 About Me:
${nepoStore.data.aboutme.replace(/(?:\r\n|\r|\n)/g, "<br>")}
...
${nepoStore.data.tech
  .join(" ")
  .replaceAll("for-the-badge", nepoStore.data.badge_theme)}
`;
    }
  ...
  `;
    ...
    finaldata = `${finaldata}
<!-- Proudly created with NEPO ( https://neporeadme.vercel.app ) -->`;
    nepStore.data.finalData = finaldata;
  }
  return useObserver(() => (
    <>
      {isVisible ? (
        ...
        <div className="flex flex-col items-center fade-on-appear">
          ...
          <img
            ...
            draggable="false"
            id="toprepo"
            src={`https://github-contributor-stats.vercel.app/api?username=${nepoStore.data.username}&limit=5&theme=${topRepoTheme}&combine_all_yearly_contributions=true`}
            alt=""
          />
