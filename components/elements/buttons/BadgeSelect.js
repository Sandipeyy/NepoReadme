...
export default function BadgeSelect({ label, url}) {
  ...
  const [isAdded, setIsAdded] = useState(nepoStore.data.tech.toString().includes(url));
  function onClickFun() {
    if (isAdded) {
      setIsAdded(false);
      //   for()
      var badgeIndex = nepoStore.data.tech.indexOf(url);
      nepoStore.data.tech.splice(badgeIndex, 1);
      //   console.log(data);
    } else {
      setIsAdded(true);
      nepoStore.data.tech.push(url);
      //   console.log(data);
    }
  }
  return (
